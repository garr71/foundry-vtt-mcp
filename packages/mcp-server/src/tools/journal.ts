...
/**
 * Chat narration and journal display tools.
 *
 * NOTE: on the pre-v14 `master` line this class also held `set-quest-visibility` and
 * `set-quest-checklist-item`. Both are hard-dependent on the Simple Quest module, which
 * changed storage layers between 3.0.20 and 5.1.4, so re-landing them is a rebuild rather
 * than a re-port. They are deliberately omitted here and scheduled for Phase 7a.
 * See docs/V14_MIGRATION_PLAN.md.
 */
export class JournalTools {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            speaker: {
              type: 'string',
              description:
                'Speaker name shown in chat. Use an actor name for NPC dialogue, or omit for GM narration.',
            },
            whisper: {
              type: 'boolean',
              description:
                'If true, message is visible only to GM clients. Default: false (public).',
              default: false,
            },
          },
          required: ['content'],
        ...
        inputSchema: {
          ...
          properties: {
            ...
            page: {
              type: 'string',
              description:
                'Optional: name or partial name of a specific page within the journal. Opens in single-page view showing only that page.',
            },
          },
          required: ['journal'],
        },
  ...
  async handleSendChatMessage(args: any): Promise<any> {
    ...
    try {
      return await this.foundryClient.query('foundry-mcp-bridge.sendChatMessage', {
        content,
        speaker,
        whisper,
      });
  ...
  async handleShowJournalToPlayers(args: any): Promise<any> {
    ...
    try {
      return await this.foundryClient.query('foundry-mcp-bridge.showJournalToPlayers', {
        journal,
        page,
      });
    } catch (error) {
